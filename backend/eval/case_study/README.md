...
show the flagship workflow _firing_: a change that looks harmless (ingesting a few
...
glossary / cheatsheet documents** — each one textually close to _many_ user
...
and the **same code** — the _only_ variable is the corpus:
...
source demoted, and the number of questions answered by the _correct_ document at
...
the canonical source is still _somewhere_ in the top 5, just no longer first).
**A gate on Hit@5 alone would have shipped this regression.** MRR is sensitive to
_where_ in the ranking the right answer lands, so it caught what a coarse
